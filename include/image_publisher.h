...
namespace Pylon
{
  ...
  public:
    ...
    virtual void OnImageGrabbed( CInstantCamera& camera, const CGrabResultPtr& ptrGrabResult)
    {
      ...
      cv::Mat cv_img_bgr(width->GetValue(), height->GetValue(), CV_8UC3);
      ...
      if (ptrGrabResult->GrabSucceeded())
      {
        ...
        cv_img_bgr = cv::Mat(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8UC3,(uint8_t*)pylon_image_.GetBuffer());
        //cv::cvtColor(cv_img_bgr, cv_img_rgb, CV_BGR2RGB);
        //cv::cvtColor(cv_img_rgb, cv_img_bgr, CV_RGB2BGR);
        sensor_msgs::ImagePtr image = cv_bridge::CvImage(std_msgs::Header(), "bgr8", cv_img_bgr).toImageMsg();
